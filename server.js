...
io.on('connection', function (socket) {
  ...
  socket.on('getPlanets', function (data) {
    planetsDataHandler(data, socket);
  });
  socket.on('getHouses', function (data) {
    housesDataHandler(data, socket);
  });
  //socket.on('getPlanets', function (data) {
...
function planetsDataHandler(data, socket) {
  console.log(data);
  console.log(data.date);
...
function housesDataHandler(data, socket) {
  console.log(data);
  console.log(data.date);
  ...
  geo = data.geo || geo;
    ...
    swisseph.swe_julday(date.year, date.month, date.day, date.hour, swisseph.SE_GREG_CAL, function (julday_ut) {
    swisseph.swe_houses(julday_ut, geo.lat, geo.long, 'K', function (result) {
      assert (!result.error, result.error);
      //console.log ('Houses for date:', result);
      socket.emit('Houses', result );
    });
  });
  //var flag = swisseph.SEFLG_SPEED | swisseph.SEFLG_MOSEPH | swisseph.SEFLG_RADIANS;// | swisseph.SEFLG_XYZ;
  //swisseph.swe_julday(date.year, date.month, date.day, date.hour, swisseph.SE_GREG_CAL, function (julday_ut) {
