...
// UI display:
 function Axes(scene, objectsArray) {
  ...
  this.geometry.vertices.push(new THREE.Vector3(0, 0.0001, 0));
  this.geometry.vertices.push(new THREE.Vector3(0, 0, 0));
...
function Houses(scene, objectsArray, data) {
  this.name =  "houses";
  this.geometry = new THREE.Geometry();
  ...
  for (var i = 0; i < this.houses.length; i++) {
    var R = 50;
    var x = -R * Math.cos(this.houses[i] * Math.PI / 180);
    var y = R * Math.sin(this.houses[i] * Math.PI / 180);
    this.geometry.vertices.push(new THREE.Vector3(0, 0, 0));
    this.geometry.vertices.push(new THREE.Vector3(x, 0, y));
  }

  this.material = new THREE.LineBasicMaterial({linewidth: 2, color: 0xfff000});// , side: THREE.DoubleSide
  this.houses = new THREE.Line(this.geometry, this.material);
...
Houses.prototype = {
  ...
  geometry: 0,
  material: 0,
  houses: 0, // The actual THREE.Object3D itself

  // Object shape and animation parameters:
  angle: 0,

  update: function () {
    //this.houses.rotation.x += 0.5 * Math.PI / 180;
  },
  draw: function () {
    // probably don't need this...
  }
...
function Ascendant(scene, objectsArray, data) {
  this.name =  "houses";
  this.geometry = new THREE.Geometry();
  this.ascendant = data.kochCoAscendant;

  var R = 0.01;
  var x = -R * Math.cos(this.ascendant * Math.PI / 180);
  var y = R * Math.sin(this.ascendant * Math.PI / 180);
  this.geometry.vertices.push(new THREE.Vector3(0, 0, 0));
  this.geometry.vertices.push(new THREE.Vector3(x, 0, y));

  this.material = new THREE.LineBasicMaterial({linewidth: 4, color: 0xff0000});// , side: THREE.DoubleSide
  this.horizon = new THREE.Line(this.geometry, this.material);
...
Ascendant.prototype = {
  ...
  geometry: 0,
  material: 0,
  horizon: 0, // The actual THREE.Object3D itself

  // Object shape and animation parameters:
  angle: 0,

  update: function () {
    //this.horizon.rotation.x += 0.5 * Math.PI / 180;
  },
  draw: function () {
    // probably don't need this...
  }
